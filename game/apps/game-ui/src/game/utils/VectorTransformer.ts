import { GameMap } from "@packages/game-data"
import Game from '../Game';

export default class VectorTransformer {
    static positionToCanvas(posX: number, posY: number, cameraX: number, cameraY: number) {
    ...
    static getPositionFromCanvas(screenX: number, screenY: number, cameraX: number, cameraY: number) {
        const tileWidth = GameMap.TILE_WIDTH
        const tileHeight = GameMap.TILE_HEIGHT
        const offsetX = Game.WIDTH * 0.5
        const offsetY = Game.HEIGHT * 0.5
