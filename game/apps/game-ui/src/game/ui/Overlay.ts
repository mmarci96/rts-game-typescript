...
class Overlay {
    ...
    displayUnitSelection(
        ...
        if (!units.size && buildings.size === 1) {
            ...
            this.displayAvailableActions(building, selectionDetails, createTrainUnitCommand)
            return;
        }
    ...
    displayAvailableActions(entity: Drawable, selectionDetails: HTMLElement, createTrainUnitCommand: (commands: Command[]) => void
    ) {
        ...
        actions.forEach((action: string) => {
            const actionCard = this.createActionCard(action, entity.entity.getId(), createTrainUnitCommand)
            selectionDetails.appendChild(actionCard)
        });
    }

    displaySelection(selectionList: Set<Drawable>, selectionDetails: HTMLElement) {
    ...
    createActionCard(action: string, id: string | null, createTrainUnitCommand: (commands: Command[]) => void) {
        const card = this.actionCard(action, id, createTrainUnitCommand)
        return card;
    }

    actionCard(actionText: string, id: string | null, createTrainUnitCommand: (commands: Command[]) => void) {
